...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_TAB) {
		...
		if (keyCode == ALLEGRO_KEY_ENTER && keyStrokes.size() == code.size()) { // cheat code: enter shift shift enter
...
void PlayScene::ConstructUI() {
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		...
		Engine::Sprite("play/turret-6.png", 1522, 136-8, 0, 0, 0, 0)
		, 1522, 136, CircleGunTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= CircleGunTurret::Price)
		preview = new CircleGunTurret(0, 0);
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		// four directions next to the point
		if (p.y - 1 >= 0 && p.y - 1 < MapHeight) {
			if (mapState[p.y - 1][p.x] == TILE_DIRT && map[p.y-1][p.x]<0) {
		...
		if (p.x + 1 >= 0 && p.x + 1 < MapWidth) {
			if (mapState[p.y][p.x + 1] == TILE_DIRT && map[p.y][p.x+1]<0) {
